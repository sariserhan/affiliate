...
def send_email(item_obj, catalog_list):
    ...
    selected_catalog = st.selectbox(
        label="Choose Category", options=catalog_list)
    if selected_catalog:
    ...
    selected_item = st.selectbox(label="Choose Item to Send",
                                 options=item_list,
                                 on_change=on_selectbox_change, args=(
                                     selected_catalog, )
                                 )
    for image_name_in_list in image_list:
        if selected_item in image_name_in_list:
            ...
            break
    if image_name:
        image = get_image(image_name=image_name,
                ...
                if not item_dict['email_sent']:
                    try:
                        email_obj.send_email(
                            recipient_email=email, item_dict=item_dict)
    ...
    with col3:
        test_email_to_send = st.text_input(
            label='Email', key='email', placeholder='Email', label_visibility='collapsed')

    with col2:
        if st.button(label='Send Test Email') and test_email_to_send:
            try:
                email_obj.send_email(
                    recipient_email=test_email_to_send, item_dict=item_dict)
